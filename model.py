...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_mode is not divisible by h"
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim = -1) # (Batch, g, seq_len, seq_len)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, source_mask, tgt_mask):
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_output, source_mask, tgt_mask):
        for layer in self.layers:
            x = layer(x, encoder_output, source_mask, tgt_mask)
        return self.norm(x)
